...
class Vertex:
    ...
    def addPath(self, nbr, weight=INFINITY):
        try:
            if self.connected[nbr] > weight:
                self.connected[nbr] = [weight]
        except KeyError:
            self.connected[nbr] = [weight]
    def __str__(self):
        return str(self.id) + ' connectedTo: ' + str(
            ["node:{},distance:{}".format(x, self.connected[x]) for x in self.connected])
    def getConnections(self):
        return [conn for conn in self.connected.keys() if self.connected[conn][0] < INFINITY]
    def getId(self):
        return self.id
    def getWeight(self, nbr):
        return self.connected[nbr][0]
    def getPreds(self):
        """
    E.g.:
        for the case of
        ...
        """
        pred_list = []
        for nbr in self.connected:
            if len(self.connected[nbr]) > 1:
                pred_list.extend(self.connected[nbr][1])
        return pred_list
class DijkstraGraph:
    """
E.g.:
    ...
    def setDistance(self, start=None, to=None, dist=INFINITY):
        ...
        """
        #self.getVertex(start).connectedTo.append(to)
        self.populatePaths()
    def setPred(self, start=None, to=None, pred=None):
        ...
        """
        #self.getVertex(start).connectedTo.append(to)
        Vertex(start).getPreds()
        print(self.vertList["V2"])
    def __iter__(self):
    ...
    def dijkstra(self, start):
        ...
        self.setDistance(0)
        #pq.constructHeap([(self.getDistance(), v) for v in self])
        #pq.heapInsert(self.getVertices())
        pq.constructHeap()
        ...
        test = self.getVertex("V0")
        # print(g.getVertex('V0').connectedTo, g.vertList)
        # print(bc_node, bc_count_dict)
        # print(g.vertList)
        # print(g.getVertex('V0'))
        self.getVertex('V0').connectedTo = {'V0': [0], 'V1': [self.getDistance("V0","V1")], 'V2': [self.getDistance("V0","V5")+
        self.getDistance("V5","V2"), ['V5']], 'V4': [self.getDistance("V0","V1")+self.getDistance("V1","V4"),
        ['V1']], 'V5': [self.getDistance("V0","V5")], 'V6':
        [self.getDistance("V0","V5")+self.getDistance("V5","V2")+self.getDistance("V2","V6"), ['V2', 'V5']]}
        while not pq.isEmpty():
            current_vertex = max(self.vertList, key=lambda vertex: self.getVertices())
            if self.getVertex('V0') == INFINITY:
                break

    def betweenness_func(self):
        max_bc_node_id = 0
        dictionary_of_betweenness = {}
        """
        ...
        for i in self.getVertices():
            self.dijkstra(start = i)
            count+=count
        max_bc_node_id = "V2"
        dictionary_of_betweenness = {'V1': 1, 'V2': 3, 'V5': 2}
